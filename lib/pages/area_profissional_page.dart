...
class _AreaProfissionalPageState extends State<AreaProfissionalPage> {
  ...
  // ─── Estilo por categoria (cor + ícone) ─────────────────────────────────────
  Map<String, dynamic> _categoriaEstilo(String categoria) {
    ...
    if (cat.contains('hidráulic') ||
        cat.contains('hidraulic') ||
        cat.contains('encanad') ||
        cat.contains('vazamento')) {
    ...
    if (cat.contains('montagem') ||
        cat.contains('marcenaria') ||
        cat.contains('móve') ||
        cat.contains('move')) {
  ...
  // ─── Abrir Google Maps com a localização do pedido ─────────────────────────
  Future<void> _abrirLocalizacao({
    double? lat,
    double? lng,
  ...
  Widget _buildEstrelas(double avaliacao) {
    final estrelasCheias = avaliacao.floor();
    final temMeia = (avaliacao - estrelasCheias) >= 0.5;
    return Row(
      ...
      builder: (ctx) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              // Cabeçalho: ícone de sucesso + título
              Row(
                children: [
                  Container(
              ...
              // Nome do cliente + selo de verificado
              const Text('Cliente',
                  style: TextStyle(
                      fontSize: 12,
              ...
              // Avaliação (só aparece se houver dado salvo)
              if (avaliacao != null) ...[
              ...
              // Endereço — agora com Expanded para quebrar linha corretamente
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: _buildAppBar(),
      // ── FIX: CustomScrollView em vez de Column fixo, evita overflow quando
      // o banner + cabeçalho não cabem na altura visível da tela ──
  ...
  Widget _buildFiltrarButton() {
    return InkWell(
      ...
      onTap: () {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Filtros chegando em breve ✨'),
  ...
  // ─── Banner de saldo (cartão premium com gradiente) ─────────────────────────
                          ...
                          const Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                SizedBox(height: 2),
                                Text(
                                  'Desbloqueie contatos e conquiste mais serviços!',
                                  style: TextStyle(
                                      color: Color(0xFFB8BAC9),
                                      fontSize: 11.5,
                                      height: 1.3),
    ...
    return Container(
      ...
      child: IntrinsicHeight(
        child: Row(
          ...
          children: [
            // Barra colorida lateral indicando a categoria
              ...
              child: Column(
                ...
                children: [
                      ...
                      children: [
                        ...
                        InkWell(
                          ...
                          onTap: () {
                            setState(() {
                              if (isFavorito) {
                                _favoritos.remove(pedidoId);
                              } else {
                                _favoritos.add(pedidoId);
                              }
                            });
                          },
                          child: Padding(
                  ...
                  if (localizacao.isNotEmpty || tempo.isNotEmpty)
                    Padding(
                      padding:
                          EdgeInsets.fromLTRB(14, 0, 14, distancia != null ? 2 : 8),
                      child: Row(
                        children: [
                  ...
                  const Divider(
                      color: Color(0xFFEFEFF2), height: 1, indent: 14, endIndent: 14),
